...
def parse_opcode(cc, DBG=True):
    ...
    output = opcode_machine(dictcode, i, input_signals,input_index,relative_base,one_output=False,DBG=False)
...
test(t1,opcode=False,expected="R,8,R,8,R,4,R,4,R,8,L,6,L,2,R,4,R,4,R,8,R,8,R,8,L,6,L,2",DBG=False)
...
def test2(cc=None, opcode=True, input_commands=[], expected=None, DBG=True):
    ...
    expected = str(expected)
    flag = (result == expected)
    print("***"+str(input_commands) + " -> "+str(result), " -> "+ str(flag) + " vs " + expected)    
    print((stop_millis-start_millis),"ms",int((stop_millis-start_millis)/1000),"s",int((stop_millis-start_millis)/1000/60),"min")


input_commands = ["A,C,A,B,C,B,C,B,A,C"+chr(10)+"L,6,L,4,R,8"+chr(10)+
"L,4,R,4,L,4,R,8"+chr(10)+
"R,8,L,6,L,4,L,10,R,8"+chr(10)+
"n"
]
