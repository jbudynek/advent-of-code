...
def parse_input(ii, DBG=True):
    ...
    for ri in range(2, rr):
        ...
        if DBG:
            print(raw_rule, rule_lhs, rule_rhs)
...
def function(ii, DBG=True):
    ...
    if DBG:
        print(match(world, 0, rules[1]))

    if DBG:
        print("0: " + world_str(world))

    # loop on 20 generations
        ...
        for index in range(-2, len(world) + 1):
            ...
            for rule in rules:
                ...
                if mm[0]:
                    ...
                    continue
        if DBG:
            print(str(gen) + ": " + world_str(new_world))
        world = new_world
