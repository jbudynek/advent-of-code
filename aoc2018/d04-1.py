...
def process_nap(world, cur_guard, falls_asleep, wakes_up, DBG=True):
    ...
    world[cur_guard][falls_asleep:wakes_up] = (
        world[cur_guard][falls_asleep:wakes_up] + 1
    )
    if DBG:
        print(cur_guard, world[cur_guard])
    return world


def find_longest_sleeper(world, DBG=True):
    if DBG:
        print("find_longest_sleeper")
    longest_nap = -1
    longest_sleeper = -1
    for sleeper in world:
        if DBG:
            print(sleeper)
        if DBG:
            print(np.sum(world[sleeper]))
        nap_len = np.sum(world[sleeper])
        if DBG:
            print(sleeper, nap_len, world[sleeper])
        if nap_len > longest_nap:
            longest_nap = nap_len
            longest_sleeper = sleeper
    return longest_sleeper


def find_most_slept_minute(world, longest_sleeper):
    naps = world[longest_sleeper]
...
def process_schedule(all_lines, DBG=True):
    ...
    for line in all_lines:
        ...
        if "begins shift" in line:
            ...
            wakes_up = int(nbs[4])
            if DBG:
                print(cur_guard, falls_asleep, wakes_up)
            world = process_nap(world, cur_guard, falls_asleep, wakes_up, DBG)
        # if(DBG):print(world)
    longest_sleeper = find_longest_sleeper(world, DBG)
    most_slept_minute = find_most_slept_minute(world, longest_sleeper)
