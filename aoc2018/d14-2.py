...
def boom(input_val, DBG = True):
    ...
    while (nb_scores<fff):
        # combine recipes = sum scores of all indexes and append each number at the end of scores
        ...
        for num in str(new_recipe):
            ...
            if(nb_scores%1000000==0):print(nb_scores)
        for ii in range(len(indexes)):
            indexes[ii] = (1+indexes[ii]+int(scores[indexes[ii]]))%nb_scores
        if DBG:print(''.join(scores),indexes)
