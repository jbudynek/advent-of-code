...
def start_job(worker, node, tick, world, DBG=True):
    if DBG:
        print("**start_job ", worker, node, tick)
...
def get_next_available_node(world, DBG=True):
    ...
    for node in world.nodes:
        if len(list(world.predecessors(node))) == 0:
            if node not in worker_to_node.values():
                ret.append(node)
    if DBG:
        print("available " + str(ret))
    if len(ret) == 0:
        return None
    else:
        return ret[0]
