...
def speak_next_number(turn,latest_spoken_number,last_seen, DBG=True):
    if DBG:print(turn,latest_spoken_number,last_seen)
    if latest_spoken_number not in last_seen:
        if DBG:print("never seen number ",latest_spoken_number)
        last_seen[latest_spoken_number] = (-1,turn)
        return (0,turn+1,last_seen)
    else:
        if DBG:print("already seen number ",latest_spoken_number)
        ls = last_seen[latest_spoken_number] 
        if (ls[0]==-1):
            if DBG:print("number ",latest_spoken_number," seen only once ",ls)
            ...
            if DBG:print("number ",latest_spoken_number," seen twice ",ls)
            delta = ls[1]-ls[0]
...
def boom(input_val, DBG=True):
    ...
    if DBG:print(last_seen,turn,"th number spoken is ",number)

    while turn <2020:    
        new_number,new_turn,last_seen = speak_next_number(turn,number,last_seen,DBG=False)        
        number,turn = new_number,new_turn 
        if DBG:print(turn,"th number spoken is ",number)
