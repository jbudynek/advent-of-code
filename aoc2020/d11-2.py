...
def count_in_dir(world, x_min, x_max, y_min, y_max, x, y, dx, dy, DBG):
    ...
    cur_y = y
    while (
        (cur_x >= x_min) and (cur_x <= x_max) and (cur_y >= y_min) and (cur_y <= y_max)
    ):
        cur_x = cur_x + dx
        cur_y = cur_y + dy
...
def count_occupied(world, x_min, x_max, y_min, y_max, x, y, DBG):
    ...
    for (dx, dy) in [
        ...
        occ = occ + count_in_dir(world, x_min, x_max, y_min, y_max, x, y, dx, dy, DBG)
    return occ
...
def tick(world, x_min, x_max, y_min, y_max, DBG=False):
    ...
    for x in range(x_min, x_max + 1):
        for y in range(y_min, y_max + 1):
            if (x, y) in world:
                occ = count_occupied(world, x_min, x_max, y_min, y_max, x, y, DBG)
                if world[(x, y)] == "L":
                    if occ == 0:
                        new_world[(x, y)] = "#"
                    else:
