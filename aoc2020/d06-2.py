...
def end_cur_group(
    groups_answers, groups_people, cur_group_answers, cur_group_people, nb_yes, DBG=True
):
    groups_answers.append(cur_group_answers)
    groups_people.append(cur_group_people)

    if DBG:
        print(cur_group_answers, len(cur_group_answers), cur_group_people)
...
def boom(input_val, DBG=True):
    ...
    for line in input_val:
        if line == "":
            ...
            for q in line:
                if q not in cur_group_answers:
                    cur_group_answers[q] = 0
                cur_group_answers[q] = cur_group_answers[q] + 1

    nb_yes = end_cur_group(
        groups_answers, groups_people, cur_group_answers, cur_group_people, nb_yes, DBG
    )
