...
def get_nb_comb(nb_adapters, DBG = True):
    if nb_adapters==1: return 1
    if nb_adapters==2: return 1
    if nb_adapters==3: return 2
    if nb_adapters==4: return 4
    if nb_adapters==5: return 7
    print("***",nb_adapters)
    return -1

def boom(input_val, DBG = True):

    # find chunks of 1-separated adapters, bounded on the left and right by separation of 3
    ...
    while right_idx<ln:
        ...
        if DBG:print(numbers[left_idx:right_idx+1],left_idx,right_idx)

        nb_adapters = right_idx-left_idx
        if DBG:print(nb_adapters)
        nbc = get_nb_comb(nb_adapters,DBG)
        if DBG:print("nbc",nbc)
        nb_comb = nb_comb * nbc
