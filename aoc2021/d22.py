...
class Box:
    ...
    def get_intersection_with(self, box2):
        ...
        zmin = max(self.zmin, box2.zmin)
        zmax = min(self.zmax, box2.zmax)
...
def process_reboot_instructions(input_val, small_region):
    all_boxes = []

    for lll in input_val:
        (on, xmin, xmax, ymin, ymax, zmin, zmax) = parse_line(lll)

        current_box = Box(xmin, xmax, ymin, ymax, zmin, zmax, on)

        if small_region and not current_box.is_eligible():
            continue

        intersections = []

        for box in all_boxes:
            if current_box.intersects(box):
                intersection = current_box.get_intersection_with(box)
                intersections.append(intersection)

        for intersection in intersections:
            all_boxes.append(intersection)

        if on:
            all_boxes.append(current_box)

    return sum(box.volume() for box in all_boxes)


def boom_part1(input_val, DBG=True):
    return process_reboot_instructions(input_val, True)


def boom_part2(input_val, DBG=True):
    return process_reboot_instructions(input_val, False)
...
tt1 = """on x=10..12,y=10..12,z=10..12
