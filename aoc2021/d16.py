...
def decode_bin(bline, cursor):
    n = Node()
    n.version = int(bline[cursor + 0 : cursor + 3], 2)  # noqa
    n.type_id = int(bline[cursor + 3 : cursor + 6], 2)  # noqa
    n.sub_nodes = []
    if n.type_id == 4:
        cursor += 6
        bnum = ""
        while True:
            chunk = bline[cursor : cursor + 5]  # noqa
            bnum += chunk[1:5]
            cursor += 5
            if chunk[0] == "1":
                continue
        ...
        if length_type_id == "0":
            total_length_in_bits = int(bline[cursor + 7 : cursor + 7 + 15], 2)  # noqa
            c0 = cursor + 7 + 15
            cursor = c0
            ...
            number_of_sub_packets_immediately_contained = int(
                bline[cursor + 7 : cursor + 7 + 11], 2  # noqa
            )
            nb = 0
            cursor = cursor + 7 + 11
