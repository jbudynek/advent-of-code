...
def parse(world, ss, DBG=True):
    ...
    if ss.startswith("turn off"):
        world[x0 : x1 + 1, y0 : y1 + 1] = np.maximum(  # noqa
            world[x0 : x1 + 1, y0 : y1 + 1] - 1, 0  # noqa
        )
    elif ss.startswith("turn on"):
        world[x0 : x1 + 1, y0 : y1 + 1] = world[x0 : x1 + 1, y0 : y1 + 1] + 1  # noqa
    elif ss.startswith("toggle"):
        world[x0 : x1 + 1, y0 : y1 + 1] = world[x0 : x1 + 1, y0 : y1 + 1] + 2  # noqa
    else:
        print("**" + ss)
        sys.exit()
    return world
