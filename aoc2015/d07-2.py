...
def get_wire_value(wire,wire_to_formula,wire_to_value, DBG=False):
    if DBG: print(wire, wire_to_value)
    ...
    try:
        ...
        lll = formula.split(" ")
        if len(lll)==1: # assign
            wire_to_value[wire] = get_wire_value(lll[0],wire_to_formula,wire_to_value, DBG)
        elif len(lll)==2: # NOT
            wire_to_value[wire] = ~get_wire_value(lll[1],wire_to_formula,wire_to_value, DBG)
        elif lll[1] == "AND":
             wire_to_value[wire] = get_wire_value(lll[0],wire_to_formula,wire_to_value, DBG) & get_wire_value(lll[2],wire_to_formula,wire_to_value, DBG)
        elif lll[1] == "OR":
             wire_to_value[wire] = get_wire_value(lll[0],wire_to_formula,wire_to_value, DBG) | get_wire_value(lll[2],wire_to_formula,wire_to_value, DBG)
        elif lll[1] == "LSHIFT":
             wire_to_value[wire] = get_wire_value(lll[0],wire_to_formula,wire_to_value, DBG) << int(lll[2])
        elif lll[1] == "RSHIFT":
             wire_to_value[wire] = get_wire_value(lll[0],wire_to_formula,wire_to_value, DBG) >> int(lll[2])
...
def boom(input_val, DBG = True):
    ...
    if DBG: print(wire_to_formula)

    wire_to_formula["b"] = 3176 # result from part 1

    for wire, formula in wire_to_formula.items():
        if wire in wire_to_value:
            pass
        else:
            wire_to_value[wire] = get_wire_value(wire,wire_to_formula,wire_to_value, DBG)
